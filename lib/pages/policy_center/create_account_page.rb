...
# Model of the WRG account creation page
class CreateAccountPage < PolicyCenterPage
  ...
  # Given an organization code, manipulate the UI to select it
  def organization_code=(val)
    organization_element.focus
    search_organization_element.click!
    wait_for_ajax
    OrganizationSearchPage.new(browser).select_organization_by_code(val.to_s)
  end

  # Given an array of hazard grades add them.
  #
  # Example YAML:
  #
  # hazard_grades:
  ...
  # Will populate the page, press update and accept the address verification
  def populate_and_update(data = nil)
    ...
    update
    Watir::Wait.until { address_verification_update? || account_summary_screen_header? }
    address_verification_update if address_verification_update?
  end
end
