...
module PageObject
  ...
  module Accessors
    ...
    def text_field(name, identifier={:index => 0}, &block)
      have_hooks = identifier.key?(:hooks)
      standard_methods(name, identifier, 'text_field_for', &block)
      define_method(name) do
        return platform.text_field_value_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").value
      end
      define_method("#{name}=") do |value|
        return platform.text_field_value_set(identifier.clone, value) unless block_given? || have_hooks
    ...
    def hidden_field(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.hidden_field_value_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").value
      end
    end
    ...
    def text_area(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.text_area_value_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").value
      end
      define_method("#{name}=") do |value|
        return platform.text_area_value_set(identifier.clone, value) unless block_given? || have_hooks
        self.send("#{name}_element").value = value
      end
    end
    ...
    def select_list(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.select_list_value_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").value
      end
      define_method("#{name}=") do |value|
        return platform.select_list_value_set(identifier.clone, value) unless block_given? || have_hooks
    ...
    def link(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.click_link_for identifier.clone unless block_given? || have_hooks
    ...
    def checkbox(name, identifier={:index => 0}, &block)
      ...
      define_method("check_#{name}") do
        return platform.check_checkbox(identifier.clone) unless block_given? || have_hooks
        self.send("#{name}_element").check
      end
      define_method("uncheck_#{name}") do
        return platform.uncheck_checkbox(identifier.clone) unless block_given? || have_hooks
        self.send("#{name}_element").uncheck
      end
      define_method("#{name}_checked?") do
        return platform.checkbox_checked?(identifier.clone) unless block_given? || have_hooks
    ...
    def radio_button(name, identifier={:index => 0}, &block)
      ...
      define_method("select_#{name}") do
        return platform.select_radio(identifier.clone) unless block_given? || have_hooks
        self.send("#{name}_element").select
      end
      define_method("#{name}_selected?") do
        return platform.radio_selected?(identifier.clone) unless block_given? || have_hooks
    ...
    def button(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.click_button_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").click
      end
    end
    ...
    def div(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.div_text_for identifier.clone unless block_given? || have_hooks
    ...
    def span(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.span_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def table(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.table_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def cell(name, identifier={:index => 0}, &block)
      ...
      define_method("#{name}") do
        return platform.cell_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def row(name, identifier={:index => 0}, &block)
      ...
      define_method("#{name}") do
        return platform.row_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def image(name, identifier={:index => 0}, &block)
      ...
      define_method("#{name}_loaded?") do
        return platform.image_loaded_for identifier.clone unless block_given? || have_hooks
    ...
    def list_item(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.list_item_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def unordered_list(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.unordered_list_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def ordered_list(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.ordered_list_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def h1(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.h1_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def h2(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.h2_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def h3(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.h3_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def h4(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.h4_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def h5(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.h5_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def h6(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.h6_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def paragraph(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.paragraph_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def file_field(name, identifier={:index => 0}, &block)
      ...
      define_method("#{name}=") do |value|
        return platform.file_field_value_set(identifier.clone, value) unless block_given? || have_hooks
        self.send("#{name}_element").value = value
      end
    end
    ...
    def label(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.label_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def area(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.click_area_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").click
      end
    end
    ...
    def b(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.b_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    ...
    def i(name, identifier={:index => 0}, &block)
      ...
      define_method(name) do
        return platform.i_text_for identifier.clone unless block_given? || have_hooks
        self.send("#{name}_element").text
      end
    end
    alias_method :icon, :i
    ...
    # @param optional block to be invoked when element method is called
    #
    def element(name, tag=:element, identifier={ :index => 0 }, hooks, &block)
      #
